...
class ActionHead(BaseModule):
	...
	def __init__(self, in_channels, hidden_dim1=2048, hidden_dim2=1024, init_cfg=None):
	...
	def loss(self, pred, target):
		...
		l0 = self.loss_fn(pred[:, 0], t[:, 0])  # throttle loss
		l1 = self.loss_fn(pred[:, 1], t[:, 1])  # steer loss
		return dict(loss_throttle=l0, loss_steer=l1, loss_action=l0 + l1)
